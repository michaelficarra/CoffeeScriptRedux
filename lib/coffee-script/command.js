...
void function () {
  var $0, additionalArgs, arg, args, cache$, cache$1, cache$3, cache$4, CoffeeScript, concat, cscodegen, escodegen, foldl, formatOptions, fs, humanReadable, input, inspect, leftColumnWidth, longOptionArguments, longParameterArguments, match, maxWidth, numberLines, o, Optimiser, optionArguments, optionMap, optionRows, options, opts, parameterArguments, parameterRows, path, pkg, positionalArgs, Preprocessor, processInput, reLongOption, reLongParameter, reShortOptions, reShortOptionsShortParameter, reShortParameter, row, rows, shortOptionArguments, shortParameterArguments, uglifyjs, wrap;
  fs = require('fs');
  path = require('path');
  ...
  if (options.help) {
    ...
    processInput = function (err) {
      ...
      if (options['source-map']) {
        try {
          result = CoffeeScript.sourceMap(result, null != options.input ? options.input : options.cli && 'cli' || 'stdin');
        } catch (e$3) {
          e = e$3;
          console.error(e.stack || e.message);
          process.exit(1);
        ...
        try {
          result = CoffeeScript.js(result, {minify: false});
        } catch (e$4) {
          e = e$4;
          console.error(e.stack || e.message);
          process.exit(1);

...
var CoffeeScript, cscodegen, esmangle, fs, humanReadable, input, input, inputSource, inspect, normalizeURLs, normalizeURLs, numberLines, Optimiser, options, optionsProcessor, path, Preprocessor, processInput, Repl, reSourceRoot, runMain, sourceReferenceRoot, sourceReferenceRootReplacement, sourceReferenceRootReplacement, writeOutput, writeOutput;
fs = require('fs');
path = require('path');
cache$ = require('./helpers');
numberLines = cache$.numberLines;
humanReadable = cache$.humanReadable;
writeOutput = cache$.writeOutput;
normalizeURLs = cache$.normalizeURLs;
...
cache$1 = (null != options['normalize-urls'] ? options['normalize-urls'] : '').split(':');
sourceReferenceRoot = cache$1[0];
sourceReferenceRootReplacement = 2 <= cache$1.length ? [].slice.call(cache$1, 1) : [];

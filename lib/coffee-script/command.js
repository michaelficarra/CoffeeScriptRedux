...
if (options.minify && !(options.compile || options["eval"])) {
  console.error('Error: --minify does not make sense without --compile or --eval');
  process.exit(1);
}

if (options.bare && !(options.compile || options["eval"])) {
...
if (options.help) {
  ...
  processInput = function(err) {
    ...
    if (options.debug && (result != null)) {
      console.error("### " + (options.optimise ? 'OPTIMISED' : 'PARSED') + " ###");
      console.error(inspect(result.toJSON()));
    }

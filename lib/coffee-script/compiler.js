...
var CS, JS, any, beingDeclared, collectIdentifiers, concatMap, declarationsNeededFor, difference, enabledHelpers, envEnrichments, exports, expr, fn, foldl1, forceBlock, genSym, h, helperNames, helpers, inlineHelpers, jsReserved, makeReturn, makeVarDeclaration, map, needsCaching, nub, statementNodes, stmt, union, usedAsExpression, _ref, _ref1, _ref2,
  __indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; },
  __slice = [].slice,
  __hasProp = {}.hasOwnProperty;

_ref = require('./functional-helpers'), any = _ref.any, concatMap = _ref.concatMap, difference = _ref.difference, foldl1 = _ref.foldl1, map = _ref.map, nub = _ref.nub, union = _ref.union;
...
genSym = (function() {
  ...
  genSymCounters = {};
  return function(pre) {
    return new JS.GenSym(pre, {}.hasOwnProperty.call(genSymCounters, pre) ? ++genSymCounters[pre] : genSymCounters[pre] = 0);
  };
})();
...
helpers = {
  isOwn: function() {
    ...
    functionBody = forceBlock(makeReturn(new JS.CallExpression(new JS.MemberExpression(false, hop, new JS.Identifier('call')), args)));
    return new JS.FunctionDeclaration(helperNames.isOwn, params, functionBody);
  }
};
...
exports.Compiler = (function() {
  ...
  defaultRules = [
    [
      CS.Program, function(_arg) {
        var block, decls, inScope, program;
  ...
  Compiler.prototype.compile = (function() {
    ...
    generateSymbols = (function() {
      var format, generatedSymbols, replaceGenSym;
      generatedSymbols = {};
      format = function(a, b) {
        return "" + a + "$" + (b || '');
      };
      replaceGenSym = function(node, usedSymbols, nsCounters) {
        var formatted, key, replacement;
        key = "" + node.ns + "$" + node.uniqueId;
        replacement = (function() {
          var _ref3;
          if (key in generatedSymbols) {
            return replacement = generatedSymbols[key];
          } else {
            if ({}.hasOwnProperty.call(nsCounters, node.ns)) {
              ++nsCounters[node.ns];
            } else {
              nsCounters[node.ns] = 0;
            }
            while (_ref3 = (formatted = format(node.ns, nsCounters[node.ns])), __indexOf.call(usedSymbols, _ref3) >= 0) {
              ++nsCounters[node.ns];
            }
            return generatedSymbols[key] = formatted;
          }
        })();
        return new JS.Identifier(replacement);
      };
      return function(node, usedSymbols, nsCounters) {
        var childName, decls, k, n, newNode, s, v, _i, _len, _nsCounters, _ref3, _usedSymbols;
        if (usedSymbols == null) {
          usedSymbols = [];
        }
        if (nsCounters == null) {
            ...
            if (__indexOf.call(node.listMembers, childName) >= 0) {
              ...
              for (_j = 0, _len1 = _ref4.length; _j < _len1; _j++) {
                ...
                if (n["instanceof"](JS.GenSym)) {
                  ...
                  _usedSymbols = (function() {
                    var _k, _len2, _results1;
                    _results1 = [];
                    for (_k = 0, _len2 = usedSymbols.length; _k < _len2; _k++) {
                      s = usedSymbols[_k];
                      _results1.push(s);
                    }
                    return _results1;
                  ...
                  _results.push(generateSymbols(n, usedSymbols, nsCounters));
                }
              }
              return _results;
            } else {
              if (node[childName]["instanceof"](JS.GenSym)) {
                ...
                return generateSymbols(node[childName], usedSymbols, nsCounters);
              }
            }
          })();
        }
    ...
    return function(ast) {
      ...
      return generateSymbols(jsAST, collectIdentifiers(jsAST));
    };
  })();

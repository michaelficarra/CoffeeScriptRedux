...
assignment = function(assignee, expression, valueUsed) {
  var assignments, e, i, m, membersName, numericallyIndexed, propName, _i, _len, _ref3;
  if (valueUsed == null) {
    valueUsed = false;
  }
  switch (false) {
    case !assignee["instanceof"](JS.ArrayExpression, JS.ObjectExpression):
      assignments = [];
      numericallyIndexed = assignee["instanceof"](JS.ArrayExpression);
      membersName = numericallyIndexed ? 'elements' : 'properties';
      e = expression;
      if (valueUsed || assignee[membersName].length > 1) {
        e = genSym('cache');
        assignments.push(new JS.AssignmentExpression('=', e, expression));
      }
      _ref3 = assignee[membersName];

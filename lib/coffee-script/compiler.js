...
generateSoak = (function() {
  var fn;
  fn = function(node) {
    var ctor, e, memberName, sym, tests, typeofTest, _ref3, _ref4, _ref5, _ref6, _ref7;
    switch (false) {
      case !node["instanceof"](CS.MemberAccessOp, CS.ProtoMemberAccessOp):
        _ref3 = fn(node.expression), tests = _ref3[0], e = _ref3[1];
        return [tests, new node.constructor(e, node.memberName)];
      case !node["instanceof"](CS.DynamicMemberAccessOp, CS.DynamicProtoMemberAccessOp):
        _ref4 = fn(node.expression), tests = _ref4[0], e = _ref4[1];
        return [tests, new node.constructor(e, node.indexingExpr)];
      case !node["instanceof"](CS.FunctionApplication):
        _ref5 = fn(node["function"]), tests = _ref5[0], e = _ref5[1];
        return [tests, new CS.FunctionApplication(e, node["arguments"])];
      case !node["instanceof"](CS.SoakedFunctionApplication):
        _ref6 = fn(node["function"]), tests = _ref6[0], e = _ref6[1];
        typeofTest = function(e) {
          return new CS.EQOp(new CS.String('function'), new CS.TypeofOp(e));
        };
        if (needsCaching(e)) {
          sym = new CS.GenSym('cache');
          return [__slice.call(tests).concat([typeofTest(new CS.AssignOp(sym, e))]), new CS.FunctionApplication(sym, node["arguments"])];
        } else {
          return [__slice.call(tests).concat([typeofTest(e)]), new CS.FunctionApplication(e, node["arguments"])];
        }
        break;
      case !node["instanceof"](CS.SoakedMemberAccessOp, CS.SoakedProtoMemberAccessOp, CS.SoakedDynamicMemberAccessOp, CS.SoakedDynamicProtoMemberAccessOp):
        ...
        _ref7 = fn(node.expression), tests = _ref7[0], e = _ref7[1];
        if (needsCaching(e)) {
          sym = new CS.GenSym('cache');
          return [__slice.call(tests).concat([new CS.UnaryExistsOp(new CS.AssignOp(sym, e))]), new ctor(sym, node[memberName])];
        } else {
          return [__slice.call(tests).concat([new CS.UnaryExistsOp(e)]), new ctor(e, node[memberName])];
        }
        break;
      default:
        return [[], node];
  ...
  return function(node) {
    var e, tests, _ref3;
    _ref3 = fn(node), tests = _ref3[0], e = _ref3[1];
  ...
  defaultRules = [
    [
      ...
      CS.Conditional, function(_arg) {
        var alternate, ancestry, condition, consequent, inspect, _ref3;
      ...
      CS.SoakedFunctionApplication, function(_arg) {
        var compile;
      ...
      CS.MemberAccessOp, CS.SoakedMemberAccessOp, function(_arg) {
      ...
      CS.ProtoMemberAccessOp, CS.SoakedProtoMemberAccessOp, function(_arg) {
        var compile, expression;
        expression = _arg.expression, compile = _arg.compile;
        if (hasSoak(this)) {
          return expr(compile(generateSoak(this)));
      ...
      CS.DynamicMemberAccessOp, CS.SoakedDynamicMemberAccessOp, function(_arg) {
        var compile, expression, indexingExpr;
        expression = _arg.expression, indexingExpr = _arg.indexingExpr, compile = _arg.compile;
        if (hasSoak(this)) {
          return expr(compile(generateSoak(this)));
        } else {
      ...
      CS.DynamicProtoMemberAccessOp, CS.SoakedDynamicProtoMemberAccessOp, function(_arg) {
        var compile, expression, indexingExpr;
        expression = _arg.expression, indexingExpr = _arg.indexingExpr, compile = _arg.compile;
        if (hasSoak(this)) {
          return expr(compile(generateSoak(this)));

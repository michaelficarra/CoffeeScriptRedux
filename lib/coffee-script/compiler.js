...
makeReturn = function makeReturn$(node) {
  var stmts;
  if (!(null != node))
...
generateMutatingWalker = function generateMutatingWalker$(fn) {
...
declarationsNeeded = function declarationsNeeded$(node) {
  if (!(null != node))
    return [];
...
declarationsNeededRecursive = function declarationsNeededRecursive$(node) {
  if (!(null != node))
    return [];
...
collectIdentifiers = function collectIdentifiers$(node) {
  return nub(function () {
    switch (false) {
    case !!(null != node):
      return [];
...
needsCaching = function needsCaching$(node) {
  if (!(null != node))
    return false;
...
forceBlock = function forceBlock$(node) {
  if (!(null != node))
...
makeVarDeclaration = function makeVarDeclaration$(vars) {
  var decls;
...
isIdentifierName = function isIdentifierName$(name) {
  return !in$(name, jsReserved) && /^[$_a-z][$_a-z0-9]*$/i.test(name);
};
memberAccess = function memberAccess$(e, member) {
...
dynamicMemberAccess = function dynamicMemberAccess$(e, index) {
...
assignment = function assignment$(assignee, expression, valueUsed) {
...
generateSoak = function () {
  ...
  fn = function fn$(node) {
    ...
    case !node['instanceof'](CS.SoakedFunctionApplication):
      ...
      tests = cache$5[0];
      e = cache$5[1];
      typeofTest = function typeofTest$(e) {
...
helpers = {
  'extends': function () {
    ...
    protoAccess = function protoAccess$(e) {
...
exports.Compiler = function () {
  ...
  Compiler.compile = (this$ = Compiler, function compile$() {
    var cache$2;
  ...
  defaultRules = [
    ...
    [
      ...
      function () {
        var groupMembers;
        groupMembers = function groupMembers$(members) {
    ...
    [
      ...
      function () {
        var handleParam;
        handleParam = function handleParam$(param, original, block) {
          var p;
          switch (false) {
        ...
        return function (param$) {
          var alternate, ancestry, block, body, cache$2, consequent, i, index, last, name, newThis, numArgs, numParams, p, parameters, parameters_, paramName, performedRewrite, pIndex, reassignments, rewriteThis, test;
          {
            cache$2 = param$;
          ...
          name = (null != ancestry[0] ? ancestry[0]['instanceof'](CS.AssignOps) : void 0) && ancestry[0].expression === this ? ancestry[0].assignee['instanceof'](CS.StaticMemberAccessOps) ? genSym(ancestry[0].assignee.memberName) : ancestry[0].assignee['instanceof'](CS.Identifier) ? genSym(ancestry[0].assignee.data) : void 0 : void 0;
          block = forceBlock(body);
    ...
    [
      ...
      function () {
        ...
        deriveArgsFromParams = function deriveArgsFromParams$(params) {
          var args;
  ...
  Compiler.prototype.addRule = function addRule$(ctor, handler) {
    this.rules[ctor.prototype.className] = handler;
    return this;
  };
  Compiler.prototype.compile = function () {
    var defaultRule, generateSymbols, walk;
    walk = function walk$(fn, inScope, ancestry, options) {
      ...
      children.compile = function compile$(node) {
    ...
    generateSymbols = function () {
      ...
      generatedSymbols = {};
      format = function format$(pre, counter) {
        var cache$2, div, mod;
        if (pre) {
          pre = pre.replace(/[^a-z0-9_$]/gi, '_').replace(/^([0-9])/, '_$1');
          return '' + pre + '$' + (counter || '');
        } else if (counter < 26) {
      ...
      generateName = function generateName$(node, param$) {
    ...
    defaultRule = function defaultRule$() {

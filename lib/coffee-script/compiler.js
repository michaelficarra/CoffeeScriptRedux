...
void function () {
  ...
  expr = function (s) {
    var accum, alternate, block, consequent, iife, push;
    if (!(null != s))
      return s;
    if (s.isExpression) {
      return s;
      ...
      s.body.body[s.body.body.length - 1] = stmt(new JS.CallExpression(push, [
        expr(s.body.body.slice(-1)[0])
      ]));
      block = new JS.BlockStatement([
        s,
        new JS.ReturnStatement(accum)
      ]);
      iife = new JS.FunctionExpression(null, [
        accum
      ], block);
  ...
  makeReturn = function (node) {
    ...
    if (node['instanceof'](JS.BlockStatement)) {
      return new JS.BlockStatement([].slice.call(node.body.slice(0, -1)).concat([
        makeReturn(node.body.slice(-1)[0])
      ]));
    } else if (node['instanceof'](JS.SequenceExpression)) {
      return new JS.SequenceExpression([].slice.call(node.expressions.slice(0, -1)).concat([
        makeReturn(node.expressions.slice(-1)[0])
      ]));
      ...
      return new JS.SwitchCase(node.test, [].slice.call(stmts.slice(0, -1)).concat([
        makeReturn(stmts.slice(-1)[0])
      ]));
  ...
  assignment = function (assignee, expression, valueUsed) {
    ...
    default:
      return new JS.SequenceExpression(valueUsed ? [].slice.call(assignments).concat([
        e
      ]) : assignments);
    }
  };
    ...
    fn = function (node) {
      ...
      case !node['instanceof'](CS.SoakedFunctionApplication):
        ...
        if (needsCaching(e)) {
          ...
          return [
            [].slice.call(tests).concat([
              typeofTest(new CS.AssignOp(sym, e))
            ]),
      ...
      case !node['instanceof'](CS.SoakedMemberAccessOp, CS.SoakedProtoMemberAccessOp, CS.SoakedDynamicMemberAccessOp, CS.SoakedDynamicProtoMemberAccessOp):
        ...
        if (needsCaching(e)) {
          ...
          return [
            [].slice.call(tests).concat([
              new CS.UnaryExistsOp(new CS.AssignOp(sym, e))
            ]),
  ...
  helpers = {
    ...
    construct: function () {
      ...
      block = [
        ...
        new JS.ReturnStatement(new JS.ConditionalExpression(new JS.BinaryExpression('===', result, new JS.CallExpression(new JS.Identifier('Object'), [
          result
        ])), result, child))
      ];
    ...
    isOwn: function () {
      ...
      functionBody = [
        new JS.CallExpression(memberAccess(hop, 'call'), args)
      ];
    ...
    defaultRules = [
      [
        ...
        function (param$) {
          ...
          block = stmt(body);
          block = block['instanceof'](JS.BlockStatement) ? block.body : [
            block
          ];
          ...
          if (decls.length > 0)
            if (options.bare) {
              ...
              block = [
                stmt(new JS.UnaryExpression('void', new JS.CallExpression(memberAccess(new JS.FunctionExpression(null, [], new JS.BlockStatement(block)), 'call'), [
                  new JS.ThisExpression
                ])))
              ];
            }
      ...
      [
        ...
        function (param$) {
          ...
          if (null != alternate)
            cases.push(new JS.SwitchCase(null, [
              stmt(alternate)
            ]));
      ...
      [
        ...
        function (param$) {
          ...
          block = null != block ? block['instanceof'](JS.BlockStatement) ? block.body : [
            block
          ] : [];
      ...
      [
        ...
        function (param$) {
          ...
          handlers = [
            new JS.CatchClause(e, catchBlock)
          ];
      ...
      [
        ...
        function (param$) {
          ...
          accum = genSym('accum');
          body = [
            stmt(new JS.AssignmentExpression('=', accum, new JS.ArrayExpression([])))
          ];
          ...
          i = genSym('i');
          vars = new JS.VariableDeclaration('var', [
            new JS.VariableDeclarator(i, left)
          ]);
          ...
          body.push(new JS.ForStatement(vars, condition, update, stmt(new JS.CallExpression(memberAccess(accum, 'push'), [
            i
          ]))));
          ...
          return function (param$) {
            ...
            if (parameters.length > 0) {
              if (parameters.slice(-1)[0].rest) {
                ...
                ]), new JS.BlockStatement([
                  stmt(new JS.AssignmentExpression('=', paramName, new JS.ArrayExpression([])))
                ]));
          ...
          if (this.boundMembers.length > 0) {
            ...
            for (var i$1 = 0, length$1 = this.boundMembers.length; i$1 < length$1; ++i$1) {
              ...
              fn = new JS.FunctionExpression(null, ps, new JS.BlockStatement([
                makeReturn(new JS.CallExpression(memberAccess(protoMember, 'apply'), [
                  instance,
                  new JS.Identifier('arguments')
                ]))
              ]));
      ...
      [
        ...
        function (param$) {
          ...
          if (!ancestry[0]['instanceof'](CS.ConcatOp)) {
            leftmost = plusOp;
            while (null != leftmost.left ? leftmost.left.left : void 0) {
      ...
      [
        CS.JavaScript,
        function () {
          return new JS.CallExpression(new JS.Identifier('eval'), [
            new JS.Literal(this.data)
          ]);
        }
      ]
    ];
      ...
      generateSymbols = function () {
        ...
        return generateMutatingWalker(function (state) {
          ...
          newNode = this['instanceof'](JS.GenSym) ? (newNode = new JS.Identifier(generateName(this, state)), usedSymbols.push(newNode.name), newNode) : this['instanceof'](JS.FunctionExpression, JS.FunctionDeclaration) ? (params = concatMap(this.params, collectIdentifiers), nsCounters_ = {}, function (accum$) {
            ...
            usedSymbols: nub([].slice.call(usedSymbols).concat([].slice.call(params))),
            nsCounters: nsCounters_
          }), newNode.body = forceBlock(newNode.body), declNames = nub(difference(map(concatMap(this.body.body, declarationsNeededRecursive), function (id) {
            return id.name;

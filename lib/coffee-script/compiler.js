...
expr = function(s) {
  ...
  if (s.isExpression) {
    ...
    return new JS.CallExpression(new JS.MemberExpression(false, iife, new JS.Identifier('call')), [new JS.ThisExpression, new JS.ArrayExpression([])]);
  } else if (s["instanceof"](JS.SwitchStatement)) {
    block = new JS.BlockStatement([makeReturn(s)]);
    iife = new JS.FunctionExpression(null, [], block);
    return new JS.CallExpression(new JS.MemberExpression(false, iife, new JS.Identifier('call')), [new JS.ThisExpression]);
  } else {
...
helpers = {
  "extends": function() {
    ...
    protoAccess = function(e) {
      return new JS.MemberExpression(false, e, new JS.Identifier('prototype'));
    };
    child = new JS.Identifier('child');
    parent = new JS.Identifier('parent');
    ctor = new JS.Identifier('ctor');
    key = new JS.Identifier('key');
    block = [new JS.ForInStatement(key, parent, new JS.IfStatement(helpers.isOwn(parent, key), stmt(new JS.AssignmentExpression('=', new JS.MemberExpression(true, child, key), new JS.MemberExpression(true, parent, key))))), new JS.FunctionDeclaration(ctor, [], new JS.BlockStatement([stmt(new JS.AssignmentExpression('=', new JS.MemberExpression(false, new JS.ThisExpression, new JS.Identifier('constructor')), child))])), new JS.AssignmentExpression('=', protoAccess(ctor), protoAccess(parent)), new JS.AssignmentExpression('=', protoAccess(child), new JS.NewExpression(ctor, [])), new JS.AssignmentExpression('=', new JS.MemberExpression(false, child, new JS.Identifier('__super__')), protoAccess(parent)), makeReturn(child)];
  ...
  defaultRules = [
    [
      ...
      CS.ForIn, function(_arg) {
        ...
        varDeclaration = new JS.VariableDeclaration('var', [new JS.VariableDeclarator(i, new JS.Literal(0)), new JS.VariableDeclarator(length, new JS.MemberExpression(false, e, new JS.Identifier('length')))]);
        if (e !== expression) {
      ...
      CS.ClassProtoAssignOp, function(_arg) {
        var a, ancestry, assignee, compile, expression, parentClass, protoMember, _i, _len;
        ...
        if (this.assignee.data === 'constructor' && this.expression["instanceof"](CS.Functions)) {
          ...
          for (_i = 0, _len = ancestry.length; _i < _len; _i++) {
            ...
            if (!a["instanceof"](CS.SeqOp, CS.Block)) {
          ...
          protoMember = new CS.MemberAccessOp(new CS.MemberAccessOp(parentClass.name, 'prototype'), this.assignee.data);

...
void function () {
  var beingDeclared, cache$, cleanMarkers, concat, concatMap, CS, difference, envEnrichments, envEnrichments_, foldl, humanReadable, map, nub, numberLines, usedAsExpression, usedAsExpression_;
  ...
  this.formatParserError = function (input, e) {
    var currentLineOffset, found, lines, message, numberedLines, numLinesOfContext, padSize, postLines, preLines, startLine;
    numLinesOfContext = 3;
    if (null != e.found) {
      lines = input.split('\n');
    ...
    found = null != e.found ? "'" + JSON.stringify(humanReadable(e.found)).replace(/^"|"$/g, '').replace(/'/g, "\\'").replace(/\\"/g, '"') + "'" : 'end of input';
    ...
    if (null != e.found) {
      ...
      message = [message].concat([].slice.call(preLines), ['' + Array(padSize + 1).join('^') + ' :~' + Array(e.column).join('~') + '^'], [].slice.call(postLines)).join('\n');
    }
    return message;
  };

...
var ArrayInitialiser, Block, Class, CompoundAssignOp, Conditional, ForOf, FunctionApplications, Functions, GenSym, HeregExp, Identifier, Identifiers, NegatedConditional, NewOp, Nodes, ObjectInitialiser, Primitives, Range, RegExp, RegExps, Slice, StaticMemberAccessOps, Super, Switch, While, allNodes, concat, concatMap, createNodes, difference, exports, handleLists, handlePrimitives, map, nub, union, _ref, _ref1,
...
Nodes = allNodes.Nodes, Primitives = allNodes.Primitives, CompoundAssignOp = allNodes.CompoundAssignOp, StaticMemberAccessOps = allNodes.StaticMemberAccessOps, Range = allNodes.Range, ArrayInitialiser = allNodes.ArrayInitialiser, ObjectInitialiser = allNodes.ObjectInitialiser, NegatedConditional = allNodes.NegatedConditional, Conditional = allNodes.Conditional, Identifier = allNodes.Identifier, ForOf = allNodes.ForOf, Functions = allNodes.Functions, While = allNodes.While, GenSym = allNodes.GenSym, Class = allNodes.Class, Block = allNodes.Block, NewOp = allNodes.NewOp, FunctionApplications = allNodes.FunctionApplications, RegExps = allNodes.RegExps, RegExp = allNodes.RegExp, HeregExp = allNodes.HeregExp, Super = allNodes.Super, Slice = allNodes.Slice, Switch = allNodes.Switch, Identifiers = allNodes.Identifiers;
...
CompoundAssignOp.prototype.childNodes = difference(CompoundAssignOp.prototype.childNodes, ['op']);
...
handleLists = function(ctor, listProps) {
  ctor.prototype.childNodes = difference(ctor.prototype.childNodes, listProps);
  return ctor.prototype.toJSON = function() {
    var json, listProp, p, _i, _len;
    json = Nodes.prototype.toJSON.call(this);
    for (_i = 0, _len = listProps.length; _i < _len; _i++) {
      listProp = listProps[_i];
      json[listProp] = (function() {
...
Switch.prototype.childNodes = ['expression', 'elseBlock'];

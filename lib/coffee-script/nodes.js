...
void function () {
    ...
    for (className in subclasses) {
      ...
      (function (className) {
        ...
        if (isCategory)
          createNodes(specs[1], [
            klass
          ].concat([].slice.call(superclasses)));
  ...
  Nodes.prototype['instanceof'] = function (ctors) {
    ...
    for (var i$ = 0, length$ = ctors.length; i$ < length$; ++i$) {
      ctor = ctors[i$];
      if (!in$(ctor.prototype.className, [
          this.className
        ].concat([].slice.call(superclasses))))
        continue;
      return true;
  ...
  handlePrimitives(CompoundAssignOp, [
    'op'
  ]);
  handlePrimitives(ForOf, [
    'isOwn'
  ]);
  handlePrimitives(HeregExp, [
    'flags'
  ]);
  handlePrimitives(Identifiers, [
    'data'
  ]);
  handlePrimitives(Primitives, [
    'data'
  ]);
  handlePrimitives(Range, [
    'isInclusive'
  ]);
  ...
  handlePrimitives(Slice, [
    'isInclusive'
  ]);
  handlePrimitives(StaticMemberAccessOps, [
    'memberName'
  ]);
  handleLists = function (ctor, listProps) {
    return ctor.prototype.listMembers = listProps;
  };
  handleLists(ArrayInitialiser, [
    'members'
  ]);
  handleLists(Block, [
    'statements'
  ]);
  handleLists(Functions, [
    'parameters'
  ]);
  handleLists(FunctionApplications, [
    'arguments'
  ]);
  handleLists(NewOp, [
    'arguments'
  ]);
  handleLists(ObjectInitialiser, [
    'members'
  ]);
  handleLists(Super, [
    'arguments'
  ...
  Block.wrap = function (s) {
    return new Block(null != s ? [
      s
    ] : []).r(s.raw).p(s.line, s.column);
  };

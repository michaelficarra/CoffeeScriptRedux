...
createNodes = function createNodes$(subclasses, superclasses) {
  var className, specs;
...
Nodes.fromJSON = function fromJSON$(json) {
...
Nodes.prototype.fold = function fold$(memo, fn) {
  var child;
  for (var i$ = 0, length$ = this.childNodes.length; i$ < length$; ++i$) {
    child = this.childNodes[i$];
    if (in$(child, this.listMembers)) {
...
Nodes.prototype.clone = function clone$() {
  var ctor, k, n, v;
  ctor = function ctor$() {
  };
...
Nodes.prototype.r = function r$(param$) {
  this.raw = param$;
  return this;
};
Nodes.prototype.p = function p$(param$, param$1, param$2) {
  this.line = param$;
  this.column = param$1;
  this.offset = param$2;
  return this;
...
Block.wrap = function wrap$(s) {
  return new Block(null != s ? [s] : []).r(s.raw).p(s.line, s.column);
};
Class.prototype.initialise = function initialise$() {
...
ObjectInitialiser.prototype.keys = function keys$() {
  return map(this.members, function (m) {
    return m.key;
  });
};
ObjectInitialiser.prototype.vals = function vals$() {
  return map(this.members, function (m) {
    return m.expression;
  });
};
RegExps.prototype.initialise = function initialise$(_, flags) {
  var flag;
  this.flags = {};
  for (var cache$2 = [
        'g',

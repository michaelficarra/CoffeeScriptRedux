...
var CS, any, beingDeclared, concat, concatMap, declarationsFor, difference, envEnrichments, foldl, foldl1, isFalsey, isTruthy, makeDispatcher, mayHaveSideEffects, union, usedAsExpression, walk, _ref, _ref1,
  __slice = [].slice,
...
makeDispatcher = function(defaultValue, handlers, defaultHandler) {
  var ctor, ctors, handler, handlers_, _i, _j, _len, _len1, _ref2;
  if (defaultHandler == null) {
    defaultHandler = (function() {});
  }
  handlers_ = {};
  for (_i = 0, _len = handlers.length; _i < _len; _i++) {
    _ref2 = handlers[_i], ctors = _ref2[0], handler = _ref2[1];
...
mayHaveSideEffects = makeDispatcher(false, [
  [
    ...
    [CS.AssignOp, CS.ClassProtoAssignOp, CS.CompoundAssignOp, CS.ExistsAssignOp], function(inScope) {
      return (mayHaveSideEffects(this.expr, inScope)) || (beingDeclared(this.assignee)).length > 0;
    }
  ], [
...
walk = (function() {
  ...
  walkDefault = function(fn, inScope, ancestry) {
    var child, childName, _i, _len, _ref2;
    if (inScope == null) {
      inScope = [];
    }
    if (ancestry == null) {
  ...
  return function(node, fn, inScope, ancestry) {
    ...
    return handler.call(node, fn, inScope, ancestry);
  };
})();

    ...
    processInput = function(isEnd) {
      return function(data) {
        ...
        if (this.ss.bol()) {
          ...
          if (!isEnd && ((this.ss.check(RegExp("[" + ws + "]*$"))) != null)) {
            return;
        ...
        while (false || (this.scan(/(?:[^\n'"\\\/\#`]+|\/\/?[^\/])+/)) || (this.ss.scan(/\\\n/)) || (this.ss.scan(/#(?!##[^#]).*/)) || (this.scan(/###[^#][\s\S]*?###/)) || (this.scan(/("""|''')[\s\S]*?\1/)) || (this.scan(/"(?:[^"\\]+|\\.)*"|'(?:[^'\\]+|\\.)*'/)) || (this.scan(/^\/{3}([\s\S]+?)\/{3}([imgy]{0,4})(?!\w)/)) || (this.scan(/`(?:[^`\\]+|\\.)*`/)) || false) {
          continue;
        }
        if (isEnd) {
          return processEnd();
        } else if (this.scan(RegExp("(?:[" + ws + "]*\\n)+"))) {

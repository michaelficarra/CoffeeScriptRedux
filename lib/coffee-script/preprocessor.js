// Generated by CoffeeScript 1.3.3
(function() {
  var EventEmitter, StringScanner, fs, inspect,
    __hasProp = {}.hasOwnProperty,
    __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

  fs = require('fs');

  EventEmitter = require('events').EventEmitter;

  StringScanner = require('cjs-string-scanner');

  inspect = function(o) {
    return (require('util')).inspect(o, false, 9e9, true);
  };

  this.Preprocessor = (function(_super) {
    var DEDENT, INDENT, processInput, ws;

    __extends(Preprocessor, _super);

    ws = '\\t\\x0B\\f \\xA0\\u1680\\u180E\\u2000-\\u200A\\u202F\\u205F\\u3000\\uFEFF';

    INDENT = '\uEFEF';

    DEDENT = '\uEFFE';

    function Preprocessor() {
      this.base = this.indent = null;
      this.context = [];
      this.context.peek = function() {
        if (this.length) {
          return this[this.length - 1];
        } else {
          return null;
        }
      };
      this.context.err = function(c) {
        throw new Error(("Unexpected " + (inspect(c)) + "\nStack Contents: " + inspect(this.slice(0))).replace(RegExp("" + INDENT, "g"), 'INDENT').replace(RegExp("" + DEDENT, "g"), 'DEDENT'));
      };
      this.context.observe = function(c) {
        var top;
        top = this.peek();
        switch (c) {
          case '"""':
          case '\'\'\'':
          case '"':
          case '\'':
          case '###':
          case '#':
          case '`':
          case '///':
          case '/':
            if (top === c) {
              this.pop();
            } else {
              this.push(c);
            }
            break;
          case INDENT:
          case 'regexp-[':
          case 'regexp-(':
          case 'regexp-{':
          case 'heregexp-[':
          case 'heregexp-(':
          case 'heregexp-{':
          case '#{':
          case '[':
          case '(':
          case '{':
          case '\\':
            this.push(c);
            break;
          case DEDENT:
            if (top !== INDENT) {
              this.err(c);
            }
            this.pop();
            break;
          case '\n':
            if (top !== '#') {
              this.err(c);
            }
            this.pop();
            break;
          case ']':
            if (top !== '[' && top !== 'regexp-[' && top !== 'heregexp-[') {
              this.err(c);
            }
            this.pop();
            break;
          case ')':
            if (top !== '(' && top !== 'regexp-(' && top !== 'heregexp-(') {
              this.err(c);
            }
            this.pop();
            break;
          case '}':
            if (top !== '#{' && top !== '{' && top !== 'regexp-{' && top !== 'heregexp-{') {
              this.err(c);
            }
            this.pop();
            break;
          case 'end-\\':
            if (top !== '\\') {
              this.err(c);
            }
            this.pop();
            break;
          default:
            throw new Error("undefined token observed: " + c);
        }
        return this;
      };
      this.ss = new StringScanner('');
    }

    Preprocessor.prototype.p = function(s) {
      if (s != null) {
        this.emit('data', s);
      }
      return s;
    };

    Preprocessor.prototype.scan = function(r) {
      return this.p(this.ss.scan(r));
    };

    processInput = function(isEnd) {
      return function(data) {
        var b, c, delta, level, newLevel, tok;
        if (!isEnd) {
          this.ss.concat(data);
        }
        while (this.ss.rest().length) {
          switch (this.context.peek()) {
            case null:
            case INDENT:
            case '#{':
            case '[':
            case '(':
            case '{':
              switch (this.context.peek()) {
                case '#{':
                  this.scan(/[^\n'"\\\/#`[({}]+/);
                  if (tok = this.scan(/\}/)) {
                    this.context.observe(tok);
                  }
                  break;
                case '[':
                  this.scan(/[^\n'"\\\/#`[({\]]+/);
                  if (tok = this.scan(/\]/)) {
                    this.context.observe(tok);
                  }
                  break;
                case '(':
                  this.scan(/[^\n'"\\\/#`[({)]+/);
                  if (tok = this.scan(/\)/)) {
                    this.context.observe(tok);
                  }
                  break;
                case '{':
                  this.scan(/[^\n'"\\\/#`[({}]+/);
                  if (tok = this.scan(/\}/)) {
                    this.context.observe(tok);
                  }
                  break;
                default:
                  this.scan(/[^\n'"\\\/#`[({]+/);
              }
              if (tok = this.scan(/"""|'''|\/\/\/|###|["'/`#[({\\]/)) {
                this.context.observe(tok);
                continue;
              }
              if (this.scan(RegExp("[" + ws + "]+\\#"))) {
                this.context.observe('#');
                continue;
              }
              if ((this.ss.check(RegExp("[" + ws + "\\n]*$"))) != null) {
                if (!isEnd) {
                  return;
                }
                this.scan(RegExp("[" + ws + "\\n]*$"));
                while (this.context.length && INDENT === this.context.peek()) {
                  this.context.observe(DEDENT);
                  this.p("" + DEDENT + "\n");
                }
                if (this.context.length) {
                  throw new Error('Unclosed ' + (inspect(this.context.peek())) + ' at EOF');
                }
                this.emit('end');
                return;
              } else if (0 === this.ss.pointer() || this.scan(RegExp("(?:[" + ws + "]*\\n)+"))) {
                if (this.base != null) {
                  if ((this.scan(this.base)) == null) {
                    throw new Error("inconsistent base indentation");
                  }
                } else {
                  this.scan(RegExp("(?:[" + ws + "]*\\n)+"));
                  b = this.scan(RegExp("[" + ws + "]*"));
                  this.base = RegExp("" + b);
                }
                if (this.indent != null) {
                  level = ((function() {
                    var _i, _len, _ref, _results;
                    _ref = this.context;
                    _results = [];
                    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                      c = _ref[_i];
                      if (c === INDENT) {
                        _results.push(0);
                      }
                    }
                    return _results;
                  }).call(this)).length;
                  if (this.ss.check(RegExp("(?:" + this.indent + "){" + (level + 1) + "}[^" + ws + "]"))) {
                    this.scan(RegExp("(?:" + this.indent + "){" + (level + 1) + "}"));
                    this.context.observe(INDENT);
                    this.p(INDENT);
                  } else if (level > 0 && this.ss.check(RegExp("(?:" + this.indent + "){0," + (level - 1) + "}[^" + ws + "]"))) {
                    newLevel = 0;
                    while (this.scan(RegExp("" + this.indent))) {
                      ++newLevel;
                    }
                    delta = level - newLevel;
                    while (delta--) {
                      this.context.observe(DEDENT);
                      this.p("" + DEDENT + "\n");
                    }
                  } else if (this.ss.check(RegExp("(?:" + this.indent + "){" + level + "}[^" + ws + "]"))) {
                    this.scan(RegExp("(?:" + this.indent + "){" + level + "}"));
                  } else {
                    throw new Error("invalid indentation");
                  }
                } else if (this.ss.check(RegExp("[" + ws + "]+[^" + ws + "#]"))) {
                  this.indent = this.scan(RegExp("[" + ws + "]+"));
                  this.context.observe(INDENT);
                  this.p(INDENT);
                }
              }
              break;
            case '\\':
              if ((this.ss.scan(/\n/)) || (this.scan(/./))) {
                this.context.observe('end-\\');
              }
              break;
            case '"""':
              this.scan(/(?:[^"#\\]+|""?(?!")|#(?!{)|\\.)+/);
              this.ss.scan(/\\\n/);
              if (tok = this.scan(/#{|"""/)) {
                this.context.observe(tok);
              } else if (tok = this.scan(/#{|"""/)) {
                this.context.observe(tok);
              }
              break;
            case '"':
              this.scan(/(?:[^"#\\]+|#(?!{)|\\.)+/);
              this.ss.scan(/\\\n/);
              if (tok = this.scan(/#{|"/)) {
                this.context.observe(tok);
              }
              break;
            case '\'\'\'':
              this.scan(/(?:[^'\\]+|''?(?!')|\\.)+/);
              this.ss.scan(/\\\n/);
              if (tok = this.scan(/'''/)) {
                this.context.observe(tok);
              }
              break;
            case '\'':
              this.scan(/(?:[^'\\]+|\\.)+/);
              this.ss.scan(/\\\n/);
              if (tok = this.scan(/'/)) {
                this.context.observe(tok);
              }
              break;
            case '###':
              this.scan(/(?:[^#]+|##?(?!#))+/);
              if (tok = this.scan(/###/)) {
                this.context.observe(tok);
              }
              break;
            case '#':
              this.scan(/[^\n]+/);
              if (tok = this.scan(/\n/)) {
                this.context.observe(tok);
              }
              break;
            case '`':
              this.scan(/[^`]+/);
              if (tok = this.scan(/`/)) {
                this.context.observe(tok);
              }
              break;
            case '///':
              this.scan(/(?:[^[/#\\]+|\/\/?(?!\/)|\\.)+/);
              if (tok = this.scan(/#{?|\/\/\/|\\/)) {
                this.context.observe(tok);
              } else if (tok = this.scan(/\[/)) {
                this.context.observe("heregexp-" + tok);
              }
              break;
            case 'heregexp-[':
              this.scan(/(?:[^\]\\#]+|#(?!#))+/);
              if (tok = this.scan(/[\]\\]|#{/)) {
                this.context.observe(tok);
              }
              break;
            case '/':
              this.scan(/[^[/\\]+/);
              if (tok = this.scan(/[\/\\]/)) {
                this.context.observe(tok);
              } else if (tok = this.scan(/\[/)) {
                this.context.observe("regexp-" + tok);
              }
              break;
            case 'regexp-[':
              this.scan(/[^\]\\]+/);
              if (tok = this.scan(/[\]\\]/)) {
                this.context.observe(tok);
              }
          }
        }
        if (isEnd) {
          this.scan(RegExp("[" + ws + "\\n]*$"));
          while (this.context.length && INDENT === this.context.peek()) {
            this.context.observe(DEDENT);
            this.p("" + DEDENT + "\n");
          }
          if (this.context.length) {
            throw new Error('Unclosed ' + (inspect(this.context.peek())) + ' at EOF');
          }
          this.emit('end');
          return;
        }
        return null;
      };
    };

    Preprocessor.prototype.processData = processInput(false);

    Preprocessor.prototype.processEnd = processInput(true);

    return Preprocessor;

  })(EventEmitter);

}).call(this);

...
expr = function (s) {
  ...
  if (s.isExpression) {
    ...
    return s.becomeExpression();
  } else {
    throw new Error('expr: Cannot use a ' + s.type + ' as a value');
  }
};
isDownwardScopeBoundary = function (node) {
  return node['instanceof'](JS.FunctionExpression, JS.FunctionDeclaration) && !node.generated;
};
isUpwardScopeBoundary = function (node) {
  return node['instanceof'](JS.FunctionExpression, JS.FunctionDeclaration);
};
...
generateWalker = function (makeIdentity) {
  return function (fn) {
    return function (node) {
      var args, identity, mapper, reducer;
      args = arguments.length > 1 ? [].slice.call(arguments, 1) : [];
      mapper = function (child, nameInParent) {
        return [
          nameInParent,
          null != child ? fn.apply(child, args) : child
        ];
      };
      ...
      return mapChildNodes(node, mapper, reducer, identity, {
...
generateMutatingWalker = generateWalker(function (node) {
  return node;
});
generateCopyingWalker = generateWalker(function (node) {
...
declaredIdentifiers = function (node) {
  ...
  if (node['instanceof'](JS.Identifier)) {
    ...
    return mapChildNodes(node, declaredIdentifiers, function (a, b) {
      return a.concat(b);
    }, []);
  }
};
declarationsNeeded = function (node) {
  ...
  if (node['instanceof'](JS.AssignmentExpression) && node.operator === '=' || node['instanceof'](JS.ForInStatement) || node['instanceof'](JS.ForOfStatement)) {
...
declarationsNeededRecursive = function (node) {
  ...
  if (isDownwardScopeBoundary(node)) {
    return [];
  } else {
...
variableDeclarations = function (node) {
  ...
  if (node['instanceof'](JS.FunctionDeclaration)) {
    return [node.id];
  } else if (isUpwardScopeBoundary(node)) {
    return [];
...
collectIdentifiers = function (node) {
  return nub(function () {
    ...
    case !node['instanceof'](JS.ObjectPattern):
      return map(node.properties, function (p) {
        return collectIdentifiers(p.value);
      }).reduce(function (a, b) {
        return a.concat(b);
      }, []);
    default:
...
es6AssignmentPattern = function (assignee) {
  var elements, elt, index, props;
  if (assignee instanceof JS.ArrayExpression) {
    elements = function (accum$) {
      for (var i$ = 0, length$ = assignee.elements.length; i$ < length$; ++i$) {
        elt = assignee.elements[i$];
        index = i$;
        accum$.push(elt instanceof JS.Identifier ? elt : elt.rest ? index === assignee.elements.length - 1 ? new JS.RestElement(elt.expression) : void 0 : es6AssignmentPattern(elt));
      }
      return accum$;
    }.call(this, []);
    ...
    props = assignee.properties.map(function (p) {
      var pattern;
      if (p.value instanceof JS.Identifier) {
        ...
        return new JS.Property(p.key, pattern);
      }
    });
    if (all(props, function (elt) {
        return null != elt;
      }) && props.length > 0) {
      return new JS.ObjectPattern(props);
    }
  }
};
...
baseExtends = function (myName, copyClassProperties) {
  var block, child, ctor, parent, protoAccess;
...
helpers = {
  'extends': function () {
    var copyClassProperties, key;
    key = new JS.Identifier('key');
    copyClassProperties = function (parent, child) {
      var f;
      return new JS.ForInStatement(new JS.VariableDeclaration('var', [new JS.VariableDeclarator(key, null)]), parent, new JS.IfStatement(helpers.isOwn(parent, key), f = stmt(new JS.AssignmentExpression('=', new JS.MemberExpression(true, child, key), new JS.MemberExpression(true, parent, key)))));
    };
    return baseExtends('extends', copyClassProperties);
  },
  extendsES6: function () {
    var copyClassProperties;
    copyClassProperties = function (parent, child) {
      var condition, conditional, copyKey, func, key, propertyNames;
      propertyNames = new JS.CallExpression(memberAccess(new JS.Identifier('Object'), 'getOwnPropertyNames'), [parent]);
      key = new JS.Identifier('key');
      copyKey = new JS.AssignmentExpression('=', new JS.MemberExpression(true, child, key), new JS.MemberExpression(true, parent, key));
      condition = [
        'callee',
        'caller',
        'arguments'
      ].map(function (forbidden) {
        return new JS.BinaryExpression('!==', key, new JS.Literal(forbidden));
      }).reduce(function (a, b) {
        return new JS.BinaryExpression('&&', a, b);
      });
      conditional = new JS.IfStatement(condition, forceBlock(copyKey));
      func = funcExpr({
        params: [key],
        body: forceBlock(conditional)
      });
      return new JS.CallExpression(memberAccess(propertyNames, 'forEach'), [func]);
    };
    return baseExtends('extendsES6', copyClassProperties);
  },
  construct: function () {
  ...
  range: function () {
    var body, first, fn, i, init, isInclusive, last, step, test, update;
    ...
    body = [new JS.ForStatement(init, test, update, new JS.BlockStatement([stmt(new JS.YieldExpression(i))]))];
    fn = funcDecl({
      id: helperNames.range,
      params: [
        first,
        last,
        step,
      ...
      body: new JS.BlockStatement(map(body, stmt))
    });
    fn.generator = true;
    return fn;
  },
  inclusiveRange: function () {
    var body, first, last, step;
    first = new JS.Identifier('first');
    last = new JS.Identifier('last');
    step = new JS.Identifier('step');
    body = [makeReturn(helpers.range(first, last, step, new JS.Literal(true)))];
    return funcDecl({
      id: helperNames.inclusiveRange,
      params: [
        first,
        last,
        step
...
findES6Methods = function (classIdentifier, ctor, isDerivedClass, body) {
  var expression, foundConstructor, m, methodBody, methodIdentifier, methods, object, properties, property, propertyName, rewriteSuper, rewriteThis, safeCtorBody, statement, unmatched;
  methods = [];
  properties = [];
  unmatched = [];
  foundConstructor = false;
  methodIdentifier = function (expression) {
    var prop;
    prop = expression.left.property;
    if (prop instanceof JS.Identifier) {
      return new JS.Identifier(prop.name);
    } else if (prop instanceof JS.Literal) {
      return new JS.Identifier(prop.value);
    }
  };
  rewriteSuper = generateCopyingWalker(function () {
    if (isDownwardScopeBoundary(this)) {
      return this;
    } else if (this.toES6Super) {
      return this.toES6Super();
    } else {
      return rewriteSuper(this);
    }
  });
  if (ctor) {
    ctor = rewriteSuper(ctor);
    if (safeCtorBody = es6SafeConstructor(isDerivedClass, ctor.body)) {
      methods.push(new JS.MethodDefinition(new JS.Identifier('constructor'), funcExpr({
        params: ctor.params,
  ...
  for (var i$ = 0, length$ = body.body.length; i$ < length$; ++i$) {
    ...
    if (statement instanceof JS.FunctionDeclaration && !foundConstructor) {
    ...
    if (expression instanceof JS.AssignmentExpression && expression.operator === '=' && expression.left instanceof JS.MemberExpression) {
      ...
      propertyName = property instanceof JS.Identifier ? property.name : property instanceof JS.Literal ? property.value : void 0;
      object = expression.left.object;
      methodBody = expression.right instanceof JS.FunctionExpression ? funcExpr({
        id: expression.right.id,
        params: expression.right.params,
        defaults: expression.right.defaults,
        rest: expression.right.rest,
        body: rewriteSuper(expression.right.body)
      }) : void 0;
      if (object instanceof JS.MemberExpression && object.property.name === 'prototype' && object.object['instanceof'](JS.ThisExpression)) {
        if (methodBody) {
  ...
  rewriteThis = generateCopyingWalker(function () {
...
es6SafeConstructor = function (isDerivedClass, block) {
  var calledSuper, callsSuper, either, referencesThis, sawThis, statement;
  if (!isDerivedClass) {
    return block;
  }
  either = function (a, b) {
  ...
  referencesThis = function (node) {
    if (null != node && node instanceof JS.ThisExpression) {
      return true;
    } else {
      return mapChildNodes(node, referencesThis, either, false);
    }
  };
  callsSuper = function (node) {
    if (null != node && node instanceof JS.CallExpression && node.callee instanceof JS.Identifier && node.callee.name === 'super') {
      return true;
    } else {
      return mapChildNodes(node, callsSuper, either, false);
    }
  };
  sawThis = calledSuper = false;
  for (var i$ = 0, length$ = block.body.length; i$ < length$; ++i$) {
    statement = block.body[i$];
    sawThis = sawThis || referencesThis(statement);
...
es6SafeArrowExpression = function (parameters, defaults, rest, block) {
  var rewriteArguments, statement, usesArguments;
  usesArguments = function (node) {
    if (!(null != node)) {
      return false;
    } else if (node instanceof JS.Identifier) {
      return node.name === 'arguments';
    } else {
      return mapChildNodes(node, usesArguments, function (a, b) {
        return a || b;
      }, false);
    }
  };
  if (any(block.body, usesArguments)) {
    ...
    rewriteArguments = generateMutatingWalker(function () {
      if (this['instanceof'](JS.Identifier) && this.name === 'arguments') {
        return rest;
      } else if (isDownwardScopeBoundary(this)) {
        return this;
      } else {
        return rewriteArguments(this);
      }
    });
    for (var i$ = 0, length$ = block.body.length; i$ < length$; ++i$) {
      statement = block.body[i$];
...
describeClass = function (name, parentNode) {
  if (null != name ? name.name : void 0) {
    return 'class ' + name.name;
  }
  if (parentNode instanceof CS.AssignOp && parentNode.assignee instanceof CS.Identifier) {
...
exports.Compiler = function () {
  ...
  defaultRules = [
    ...
    [
      CS.ForIn,
      function (param$) {
        var block, body, cache$2, compile, e, es6Target, filter, i, increment, k, keyAssignee, length, op, options, rangeArgs, step, target, update, valAssignee, valPattern, varDeclaration;
        {
          cache$2 = param$;
        ...
        if (options.targetES6 && !filter) {
          es6Target = this.target['instanceof'](CS.Range) ? (rangeArgs = [
            compile(this.target.left),
            compile(this.target.right)
          ], !(step instanceof JS.Literal && step.value === 1) ? rangeArgs.push(step) : void 0, this.target.isInclusive ? helpers.inclusiveRange.apply(helpers, [].slice.call(rangeArgs)) : helpers.range.apply(helpers, [].slice.call(rangeArgs))) : step instanceof JS.Literal && step.value === 1 ? target : void 0;
          if (es6Target && valAssignee && (valPattern = valAssignee instanceof JS.Identifier ? valAssignee : es6AssignmentPattern(valAssignee))) {
            if (keyAssignee) {
              return new JS.ForOfStatement(new JS.VariableDeclaration('var', [new JS.VariableDeclarator(new JS.ArrayPattern([
                  keyAssignee,
                  valPattern
                ]))]), new JS.CallExpression(memberAccess(es6Target, 'entries'), []), forceBlock(body));
            } else {
    ...
    [
      ...
      function () {
        ...
        return function (param$) {
          var cache$2, compile, grouped, members, options;
          {
            cache$2 = param$;
          ...
          if (!options.targetES6 && any(members, function (m) {
              return m.spread;
            })) {
            ...
            return new JS.ArrayExpression(map(members, function (m) {
              return expr(m.spread ? new JS.SpreadElement(m.expression) : m);
            }));
          }
        };
      }()
    ...
    [
      ...
      function () {
        ...
        return function (param$) {
          var alternate, ancestry, arrow, block, cache$2, consequent, defaults, expression, i, index, inScope, last, newThis, numArgs, numParams, options, p, parameters_, paramName, performedRewrite, pIndex, reassignments, rest, rewriteThis, sym, test;
          var body, parameters;
          {
            cache$2 = param$;
          ...
          if (parameters.length > 0) {
            if (parameters.slice(-1)[0].rest) {
              if (options.targetES6) {
                ...
                rest = expression instanceof JS.Identifier ? new JS.Identifier(expression.name) : (sym = genSym('rest'), block.body.unshift(stmt(new JS.AssignmentExpression('=', expression, sym))), sym);
              } else {
          ...
          if (this['instanceof'](CS.BoundFunction)) {
            if (options.targetES6 && (arrow = es6SafeArrowExpression(parameters, defaults, rest, block))) {
              return arrow;
            } else {
              newThis = genSym('this');
    ...
    [
      CS.Class,
      function (param$) {
        var _, ancestry, args, block, body, c, cache$2, cache$3, classAssignment, classExpression, classIdentifier, compile, ctorBody, ctorIndex, ctorRef, helper, i, iife, instance, member, memberName, methods, nameAssignee, options, params, parent, parentIdentifier, parentRef, properties, protoAssignOp, protoMember, ps, rewriteThis, unmatched;
        var ctor, name;
        {
          cache$2 = param$;
        ...
        if (options.targetES6) {
          ...
          if (this.ctor && !this.ctor.expression['instanceof'](CS.Functions)) {
            ...
            classExpression = new JS.ClassExpression(classIdentifier instanceof JS.GenSym ? null : classIdentifier, parentIdentifier, new JS.ClassBody(methods));
            classAssignment = new JS.AssignmentExpression('=', classIdentifier, classExpression);
            return properties.length === 0 ? classIdentifier instanceof JS.GenSym ? classExpression : (classAssignment.becomeStatement = function () {
              var statement;
              statement = new JS.ClassDeclaration(classIdentifier, parentIdentifier, new JS.ClassBody(methods));
              statement.becomeExpression = function () {
                return classAssignment;
              };
              return statement;
            }, classAssignment) : (block = new JS.BlockStatement([stmt(classAssignment)].concat(map(properties, stmt)).concat(makeReturn(classIdentifier))), new JS.CallExpression(funcExpr({ body: block }).g(), []));
          }
        }
        args = [];
        params = [];
        ...
        if (null != parent) {
          ...
          args.push(parent);
          helper = options.targetES6 ? 'extendsES6' : 'extends';
    ...
    [
      CS.Super,
      function (param$) {
        var ancestry, args, cache$2, calledExprs, classAssignNode, className, classNode, classPositionInAncestry, compile, functionName, inScope, isProtoMemberAccess, isStatic, options, tagForES6;
        {
          cache$2 = param$;
        ...
        tagForES6 = function (node) {
          node.toES6Super = function () {
            var callee;
            callee = functionName === 'constructor' ? new JS.Identifier('super') : memberAccess(new JS.Identifier('super'), functionName);
            args = args.length > 0 ? map(args, expr) : [new JS.SpreadElement(new JS.Identifier('arguments'))];
        ...
        return tagForES6(isStatic ? args.length === 0 ? new JS.CallExpression(memberAccess(memberAccess(memberAccess(memberAccess(new JS.Identifier(className), '__super__'), 'constructor'), functionName), 'apply'), [
          new JS.ThisExpression,
          new JS.Identifier('arguments')
        ]) : (calledExprs = [new JS.ThisExpression].concat(map(args, expr)), new JS.CallExpression(memberAccess(memberAccess(memberAccess(memberAccess(new JS.Identifier(className), '__super__'), 'constructor'), functionName), 'call'), calledExprs)) : args.length === 0 ? new JS.CallExpression(memberAccess(memberAccess(memberAccess(new JS.Identifier(className), '__super__'), functionName), 'apply'), [
          new JS.ThisExpression,
          new JS.Identifier('arguments')
        ]) : (calledExprs = [new JS.ThisExpression].concat(map(args, expr)), new JS.CallExpression(memberAccess(memberAccess(memberAccess(new JS.Identifier(className), '__super__'), functionName), 'call'), calledExprs)));
      }
    ],
    [
      CS.FunctionApplication,
      function (param$) {
        var args, cache$2, compile, context, lhs, options;
        var fn;
        {
          cache$2 = param$;
          fn = cache$2['function'];
        ...
        if (!options.targetES6 && any(args, function (m) {
            return m.spread;
          })) {
          lhs = this['function'];
          context = new CS.Null;
          ...
          return new JS.CallExpression(expr(fn), map(args, function (a) {
            return expr(options.targetES6 && a.spread ? new JS.SpreadElement(a.expression) : a);
          }));
        }
      }
    ],
    ...
    [
      ...
      function (param$) {
        ...
        if (right['instanceof'](JS.ArrayExpression) && right.elements.length < 5 && all(right.elements, function (elt) {
            return !(elt instanceof JS.SpreadElement);
          })) {
    ...
    [
      CS.ExtendsOp,
      function (param$) {
        var cache$2, helper, left, options, right;
        {
          cache$2 = param$;
          left = cache$2.left;
          right = cache$2.right;
        ...
        helper = options.targetES6 ? 'extendsES6' : 'extends';
        return helpers[helper](expr(left), expr(right));
      }
    ],
    [
      CS.InstanceofOp,
  ...
  Compiler.prototype.compile = function () {
    ...
    generateSymbols = function () {
      ...
      generateChildSymbols = generateMutatingWalker(function (state) {
        ...
        usedSymbols = cache$2.usedSymbols;
        nsCounters = cache$2.nsCounters;
        newNode = this['instanceof'](JS.GenSym) ? (newNode = new JS.Identifier(generateName(this, state)), usedSymbols.push(newNode.name), newNode) : isDownwardScopeBoundary(this) ? (params = concatMap(this.params, collectIdentifiers), nsCounters_ = {}, function (accum$) {
          for (k in nsCounters) {

      ...
      function (param$) {
        ...
        if (options.targetES6) {
          ...
          if (unmatched.length === 0 && (!this.ctor || this.ctor.expression['instanceof'](CS.Functions)) && (!nameAssignee || nameAssignee['instanceof'](JS.Identifier))) {
            return classIdentifier['instanceof'](JS.GenSym) ? properties.length === 0 ? new JS.ClassExpression(null, parentIdentifier, new JS.ClassBody(methods)) : (block = new JS.BlockStatement([new JS.ClassDeclaration(classIdentifier, parentIdentifier, new JS.ClassBody(methods))].concat(map(properties, stmt)).concat(makeReturn(classIdentifier))), new JS.CallExpression(funcExpr({ body: block }).g(), [])) : new JS.SequenceExpression([new JS.ClassDeclaration(classIdentifier, parentIdentifier, new JS.ClassBody(methods))].concat(properties));
          }
        }
        args = [];
        params = [];

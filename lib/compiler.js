...
stmt = function (e) {
  ...
  if (e.isStatement) {
    ...
    return new JS.ClassDeclaration(e.id, e.superClass, e.body);
  } else {
...
exports.Compiler = function () {
  ...
  defaultRules = [
    ...
    [
      CS.Class,
      function (param$) {
        var _, ancestry, args, block, body, c, cache$2, cache$3, classIdentifier, compile, ctorBody, ctorIndex, ctorRef, i, iife, instance, member, memberName, methods, nameAssignee, options, params, parent, parentIdentifier, parentRef, properties, protoAssignOp, protoMember, ps, rewriteThis, safeCtor, unmatched;
        var ctor, name;
        {
          cache$2 = param$;
        ...
        if (options.targetES6) {
          ...
          if (this.ctor && !this.ctor.expression['instanceof'](CS.Functions)) {
            ...
            return properties.length === 0 ? classIdentifier['instanceof'](JS.GenSym) ? new JS.ClassExpression(null, parentIdentifier, new JS.ClassBody(methods)) : new JS.ClassExpression(classIdentifier, parentIdentifier, new JS.ClassBody(methods)) : (block = new JS.BlockStatement([new JS.ClassDeclaration(classIdentifier, parentIdentifier, new JS.ClassBody(methods))].concat(map(properties, stmt)).concat(makeReturn(classIdentifier))), new JS.CallExpression(funcExpr({ body: block }).g(), []));
          }
        }
        args = [];
        params = [];

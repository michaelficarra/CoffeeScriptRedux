...
var beingDeclared, cleanMarkers, colorize, colors, concat, concatMap, CS, difference, envEnrichments, envEnrichments_, foldl, humanReadable, map, nub, numberLines, pointToErrorLocation, supportsColors, usedAsExpression, usedAsExpression_;
...
this.pointToErrorLocation = pointToErrorLocation = function (source, line, column, numLinesOfContext) {
  var currentLineOffset, lines, marker, numberedLines, padSize, postLines, preLines, startLine;
  ...
  return [].slice.call(preLines).concat(['' + Array(padSize + 1).join('^') + ' : ' + Array(column).join(' ') + marker], [].slice.call(postLines)).join('\n');
};
...
this.colorize = colorize = function (color, str) {
  return colors[color][0] + str + colors[color][1];
};
this.supportsColors = supportsColors = function () {
  return 'undefined' !== typeof process && null != process && process.stdout.isTTY && !process.env.NODE_DISABLE_COLORS;
};
